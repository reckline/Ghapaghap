...
const Video = require("../model/video");
const Message = require("../model/Message"); // ✅ Added for unread counts
const Settings = require('../model/settings'); // ✅ Added for packs
const Notification = require('../model/Notification');
const mongoose = require("mongoose");
const axios = require("axios");
const FormData = require("form-data");
...
exports.getHomePage = async (req, res) => {
  try {
    ...
    // ✅ Admin data fetch kar rahe hain WhatsApp toggle ke liye
    const [freshUser, unreadCount, adminData] = await Promise.all([
      userId ? User.findById(userId).lean() : null,
      getUnreadCount(userId),
      User.findOne({ role: { $regex: /^admin$/i } }).lean()
    ]);
    ...
    res.render("User/home", { 
        user: freshUser, 
        unreadCount,
        adminData: adminData, // ✅ sidebar ke liye
        admin: adminData,     // ✅ backup for older templates
        creators: creatorsWithStatus,
        videos: videos || [], 
        title: "GhapaGhap - Home"
    });
...
exports.getWatchPage = async (req, res) => {
  try {
    ...
    const [freshUser, video, unreadCount, adminData] = await Promise.all([
      userId ? User.findById(userId).lean() : null,
      Video.findById(videoId).populate('uploader').lean(),
      getUnreadCount(userId),
      User.findOne({ role: { $regex: /^admin$/i } }).lean()
...
exports.getShortsPage = async (req, res) => {
  ...
  if (freshUser) freshUser.avatar = getSafeAvatar(freshUser);
  res.render("User/shorts", { user: freshUser, unreadCount, adminData, title: "Shorts" });
};
...
exports.getTrendingPage = async (req, res) => {
  ...
  if (freshUser) freshUser.avatar = getSafeAvatar(freshUser);
  res.render("User/trending", { user: freshUser, unreadCount, adminData, title: "Trending" });
};
...
exports.getTopCreators = async (req, res) => {
    try {
        ...
        const [adminData] = await Promise.all([User.findOne({ role: { $regex: /^admin$/i } }).lean()]);
...
exports.getUserSubs = async (req, res) => {
  try {
    ...
    const [me, unreadCount, adminData] = await Promise.all([
        User.findById(sessionUserId).lean(),
        getUnreadCount(sessionUserId),
        User.findOne({ role: { $regex: /^admin$/i } }).lean()
    ]);
...
exports.getProfile = async (req, res) => {
  try {
    ...
    const [freshUser, allContent, unreadCount, adminData] = await Promise.all([
      User.findById(userId).lean(),
      Video.find({ uploader: userId }).sort({ createdAt: -1 }).lean(),
      getUnreadCount(userId),
      User.findOne({ role: { $regex: /^admin$/i } }).lean()
...
exports.getEditProfile = async (req, res) => {
  try {
    const userId = req.session.user._id;
    const [user, unreadCount, adminData] = await Promise.all([
        User.findById(userId).lean(),
        getUnreadCount(userId),
        User.findOne({ role: { $regex: /^admin$/i } }).lean()
    ]);
    if (user) user.avatar = getSafeAvatar(user);
    res.render("User/editProfile", { user, unreadCount, adminData, title: "Edit Profile" });
  } catch (err) {
    res.redirect("/profile");
  }
};
...
exports.getUserDashboard = async (req, res) => {
  try {
    ...
    if (user) user.avatar = getSafeAvatar(user);
    res.render("User/dashboard", { user, unreadCount, adminData, title: "Creator Dashboard" });
  } catch (err) {
    res.status(500).send("Dashboard Error");
  }
};
...
exports.getChangePassword = async (req, res) => {
  ...
  res.render("User/changePassword", { user: freshUser, unreadCount, adminData, title: "Change Password" });
};
...
exports.getAddBankPage = async (req, res) => {
  ...
  res.render("User/addBankAccount", { user: freshUser, unreadCount, adminData, title: "Add Bank Account" });
};
...
exports.getDepositPage = async (req, res) => {
    try {
        const [adminData, unreadCount] = await Promise.all([
            User.findOne({ role: { $regex: /^admin$/i } }).lean(),
            getUnreadCount(req.session.user?._id)
        ]);
        const upiID = adminData ? adminData.upiId : "biswassaurav@okicici";
...
// ==========================================
// 6. SUBSCRIPTION PACKS
// ==========================================

exports.getSubscriptionPacks = async (req, res) => {
    try {
        const [settings, unreadCount, adminData] = await Promise.all([
